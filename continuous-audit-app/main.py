...
# ─────────────────────────────────────────────────────────────────────────────
# Routes — MIGRAÇÃO PRD (temporário — remover após o cutover)
# O app é o único com escrita em produção, então os passos do runbook
# (Setup/MIGRATION-PROD.md) rodam por aqui, com alvo EXPLÍCITO — independem
# do CA_CATALOG/CA_SCHEMA atual do app.
# ─────────────────────────────────────────────────────────────────────────────
...
_SEED_JSON_PATH = os.path.join(os.path.dirname(__file__), "seed_v1_tests.json")

_EXEC_NEW_COLS = [
    ("IncidentCountRaw", "INT"),
    ("IsSupressed",      "BOOLEAN"),
    ("IsRecurrent",      "BOOLEAN"),
    ("IsContinued",      "BOOLEAN"),
]
...
def _require_admin(user: User) -> None:
    if user.email not in SELF_REVIEW_ALLOWED:
        raise HTTPException(403, "Acesso restrito à migração")


def _migration_target(catalog: str, schema: str) -> str:
    if (catalog, schema) not in ALLOWED_MIGRATION_TARGETS:
        raise HTTPException(400, f"Alvo não permitido: {catalog}.{schema}")
    return f"{catalog}.{schema}"


def _control_tables_ddl(fq: str) -> list:
    """(nome, DDL) das 9 tabelas de controle — mesmo schema do Setup/setup-tables.sql."""
    return [
        ("tb_test_configurations", f"""
            CREATE TABLE IF NOT EXISTS {fq}.tb_test_configurations (
                test_id STRING NOT NULL, test_name STRING NOT NULL,
                output_table STRING NOT NULL, description STRING,
                responsible_area STRING, risco_id STRING,
                threshold INT NOT NULL, frequency STRING NOT NULL,
                query_type STRING NOT NULL, imports STRING, query_code STRING NOT NULL,
                status STRING NOT NULL, category STRING, created_by STRING NOT NULL,
                reviewed_by STRING, rejection_reason STRING,
                created_at TIMESTAMP NOT NULL, updated_at TIMESTAMP NOT NULL,
                activated_at TIMESTAMP, version INT NOT NULL,
                should_activate_channel BOOLEAN
            ) USING DELTA"""),
        ("tb_test_configurations_history", f"""
            CREATE TABLE IF NOT EXISTS {fq}.tb_test_configurations_history (
                history_id STRING NOT NULL, test_id STRING NOT NULL,
                test_name STRING NOT NULL, version INT NOT NULL,
                query_type STRING, imports STRING, query_code STRING,
                status_before STRING, status_after STRING,
                changed_by STRING NOT NULL, changed_at TIMESTAMP NOT NULL,
                change_type STRING NOT NULL, comment STRING
            ) USING DELTA"""),
        ("tb_tests_executions", f"""
            CREATE TABLE IF NOT EXISTS {fq}.tb_tests_executions (
                TestName STRING, Description STRING, ResponsibleArea STRING,
                RiskId STRING, Frequency STRING, ExecutionDate TIMESTAMP,
                IncidentCount INT, IncidentCountRaw INT, ExecutionTimeSec FLOAT,
                Threshold INT, TestResult STRING, ShouldActivateChannel BOOLEAN,
                ErrorMessage STRING, IsSupressed BOOLEAN, IsRecurrent BOOLEAN,
                IsContinued BOOLEAN
            ) USING DELTA"""),
        ("tb_incident_hashes", f"""
            CREATE TABLE IF NOT EXISTS {fq}.tb_incident_hashes (
                test_name STRING, execution_date TIMESTAMP, incident_hash STRING,
                row_count INT, is_suppressed BOOLEAN, is_recurrent BOOLEAN
            ) USING DELTA"""),
        ("tb_test_suppressions", f"""
            CREATE TABLE IF NOT EXISTS {fq}.tb_test_suppressions (
                suppression_id STRING NOT NULL, test_id STRING NOT NULL,
                test_name STRING, linked_entry_id STRING, linked_entry_title STRING,
                note STRING, created_by STRING, created_at TIMESTAMP, active BOOLEAN
            ) USING DELTA"""),
        ("tb_false_positives", f"""
            CREATE TABLE IF NOT EXISTS {fq}.tb_false_positives (
                fp_id STRING NOT NULL, test_name STRING, row_hash STRING,
                match_criteria STRING, marked_by STRING, marked_at TIMESTAMP,
                note STRING, active BOOLEAN
            ) USING DELTA"""),
        ("tb_false_positives_history", f"""
            CREATE TABLE IF NOT EXISTS {fq}.tb_false_positives_history (
                fp_history_id STRING NOT NULL, fp_id STRING, event_type STRING,
                test_name STRING, row_hash STRING, row_data STRING,
                note STRING, actor STRING, event_at TIMESTAMP
            ) USING DELTA"""),
        ("tb_dashboard_views", f"""
            CREATE TABLE IF NOT EXISTS {fq}.tb_dashboard_views (
                view_id STRING NOT NULL, title STRING, position INT,
                created_by STRING, created_at TIMESTAMP, updated_at TIMESTAMP
            ) USING DELTA"""),
        ("tb_dashboard_charts", f"""
            CREATE TABLE IF NOT EXISTS {fq}.tb_dashboard_charts (
                chart_id STRING NOT NULL, view_id STRING NOT NULL, title STRING,
                chart_type STRING, test_id STRING, config STRING, position INT,
                width STRING, created_at TIMESTAMP, updated_at TIMESTAMP
            ) USING DELTA"""),
    ]


@app.get("/api/admin/migration/status")
def migration_status(catalog: str, schema: str, user: User = Depends(get_user)):
    """Read-only: existência e contagem das tabelas de controle + incidentes no alvo."""
    _require_admin(user)
    fq = _migration_target(catalog, schema)
    control = {}
    for name, _ in _control_tables_ddl(fq):
        try:
            control[name] = db.query(f"SELECT COUNT(*) AS c FROM {fq}.{name}")[0]["c"]
        except Exception:
            control[name] = None  # não existe / sem acesso
    # Colunas novas do tb_tests_executions
    exec_cols = {}
    try:
        desc = db.query(f"DESCRIBE TABLE {fq}.tb_tests_executions")
        existing = {r["col_name"] for r in desc if r.get("col_name")}
        for cname, _ in _EXEC_NEW_COLS:
            exec_cols[cname] = cname in existing
    except Exception as e:
        exec_cols = {"error": str(e)[:150]}
    # Tabelas de incidentes presentes
    try:
        tables = db.query(f"SHOW TABLES IN {fq}")
        incident_tables = sorted(
            r["tableName"] for r in tables
            if r.get("tableName", "").startswith(("tb_incidents_", "tb_claude_audit_log"))
        )
    except Exception:
        incident_tables = []
    return {
        "target": fq,
        "control_tables": control,          # None = não existe
        "executions_new_columns": exec_cols,
        "incident_tables_count": len(incident_tables),
        "incident_tables": incident_tables,
        "app_current_env": f"{CATALOG}.{SCHEMA}",
    }


@app.post("/api/admin/migration/probe-write")
def migration_probe_write(catalog: str, schema: str, user: User = Depends(get_user)):
    """Testa permissão de escrita: cria e dropa uma tabela probe no alvo."""
    _require_admin(user)
    fq = _migration_target(catalog, schema)
    probe = f"{fq}.__ca_migration_probe"
    try:
        db.execute(f"CREATE TABLE IF NOT EXISTS {probe} (x INT) USING DELTA")
        db.execute(f"DROP TABLE IF EXISTS {probe}")
        return {"write_ok": True, "target": fq}
    except Exception as e:
        return {"write_ok": False, "target": fq, "error": str(e)[:300]}


@app.post("/api/admin/migration/setup-tables")
def migration_setup_tables(catalog: str, schema: str, user: User = Depends(get_user)):
    """Passo 1 — cria as tabelas de controle (IF NOT EXISTS; não toca em dados)."""
    _require_admin(user)
    fq = _migration_target(catalog, schema)
    results = {}
    for name, ddl in _control_tables_ddl(fq):
        try:
            db.execute(ddl)
            results[name] = "ok"
        except Exception as e:
    ...
    return {"status": "done" if all_ok else "partial", "target": fq, "tables": results}


@app.post("/api/admin/migration/alter-executions")
def migration_alter_executions(catalog: str, schema: str, user: User = Depends(get_user)):
    """Passo 2 — adiciona as colunas novas ao tb_tests_executions do V1 (idempotente)."""
    _require_admin(user)
    fq = _migration_target(catalog, schema)
    try:
        desc = db.query(f"DESCRIBE TABLE {fq}.tb_tests_executions")
        ...
        raise HTTPException(400, f"tb_tests_executions inacessível em {fq}: {str(e)[:200]}")
    added, results = [], {}
    for cname, ctype in _EXEC_NEW_COLS:
        if cname in existing:
            results[cname] = "já existe"
            continue
        try:
            db.execute(f"ALTER TABLE {fq}.tb_tests_executions ADD COLUMNS ({cname} {ctype})")
            results[cname] = "adicionada"
            added.append(cname)
        except Exception as e:
            results[cname] = f"error: {str(e)[:200]}"
    ok = all(not str(v).startswith("error") for v in results.values())
    return {"status": "done" if ok else "partial", "target": fq, "columns": results, "added": added}


@app.post("/api/admin/migration/seed-tests")
def migration_seed_tests(catalog: str, schema: str, user: User = Depends(get_user)):
    """Passo 3 — insere os 28 testes do V1 (frequências REAIS; idempotente por test_name)."""
    _require_admin(user)
    fq = _migration_target(catalog, schema)
    try:
        ...
        raise HTTPException(500, f"seed_v1_tests.json não encontrado no app: {e}")
    if len(tests) != 28:
        raise HTTPException(500, f"Seed inesperado: {len(tests)} testes (esperado 28)")
    ...
    for t in tests:
        try:
            db.execute(
                f"DELETE FROM {fq}.tb_test_configurations WHERE test_name = %(n)s",
                {"n": t["test_name"]}
            )
            db.execute(f"""
                INSERT INTO {fq}.tb_test_configurations (
                    test_id, test_name, output_table, description, responsible_area,
                    risco_id, threshold, frequency, query_type, imports, query_code,
                    status, category, created_by, created_at, updated_at,
                    activated_at, version, should_activate_channel
                ) VALUES (
                    %(id)s, %(name)s, %(out)s, %(desc)s, %(area)s, %(risk)s,
                    %(thr)s, %(freq)s, %(qt)s, %(imp)s, %(qc)s, %(st)s, %(cat)s,
                    'v1-migration', %(now)s, %(now)s, %(act)s, 1, %(nc)s
                )
            """, {
                "id": str(uuid.uuid4()), "name": t["test_name"], "out": t["output_table"],
                "desc": t["description"], "area": t["responsible_area"], "risk": t["risco_id"],
                "thr": int(t["threshold"]), "freq": t["frequency"], "qt": t["query_type"],
                "imp": t["imports"], "qc": t["query_code"], "st": t["status"],
                "cat": t["category"], "now": now,
                "act": now if t["status"] == "ACTIVE" else None,
                "nc": bool(t["should_activate_channel"]),
            })
            inserted += 1
        except Exception as e:
            errors[t["test_name"]] = str(e)[:200]
    return {
        "status": "done" if not errors else "partial",
        "target": fq, "inserted": inserted, "errors": errors,
        "active": sum(1 for t in tests if t["status"] == "ACTIVE"),
        "paused": sum(1 for t in tests if t["status"] == "PAUSED"),
    }


@app.get("/api/admin/migration/validate")
def migration_validate(catalog: str, schema: str, user: User = Depends(get_user)):
    """Passo 4 — checklist read-only pós-migração."""
    _require_admin(user)
    fq = _migration_target(catalog, schema)
    checks = {}
    try:
        rows = db.query(f"SELECT status, COUNT(*) AS c FROM {fq}.tb_test_configurations GROUP BY status")
        by_status = {r["status"]: r["c"] for r in rows}
        checks["configs_by_status"] = by_status
        checks["configs_ok"] = by_status.get("ACTIVE", 0) == 25 and by_status.get("PAUSED", 0) == 3
    except Exception as e:
        checks["configs_by_status"] = f"error: {str(e)[:150]}"; checks["configs_ok"] = False
    try:
        desc = db.query(f"DESCRIBE TABLE {fq}.tb_tests_executions")
        existing = {r["col_name"] for r in desc if r.get("col_name")}
        checks["executions_columns_ok"] = all(c in existing for c, _ in _EXEC_NEW_COLS)
    except Exception:
        checks["executions_columns_ok"] = False
    try:
        sample = db.query(f"""
            SELECT c.test_name, c.output_table
            FROM {fq}.tb_test_configurations c
            WHERE c.status = 'ACTIVE' LIMIT 3
        """)
        linked = {}
        for s in sample:
            try:
                r = db.query(f"SELECT COUNT(*) AS c, MAX(ArchiveDate) AS last FROM {fq}.{s['output_table']}")[0]
                linked[s["test_name"]] = {"rows": r["c"], "last_archive": r["last"]}
            except Exception as e:
                linked[s["test_name"]] = f"error: {str(e)[:120]}"
        checks["historic_sample"] = linked
    except Exception as e:
        checks["historic_sample"] = f"error: {str(e)[:150]}"
    checks["app_current_env"] = f"{CATALOG}.{SCHEMA}"
    checks["app_pointing_to_target"] = (f"{CATALOG}.{SCHEMA}" == fq)
    return checks
